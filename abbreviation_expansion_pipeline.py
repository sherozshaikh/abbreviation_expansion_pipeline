...
class AbbreviationExpansionPipeline():
  ...
  def main(self)->None:
    ...
    working_result_score_df.column = ['PROD_DESC1','PROD_DESC1','SIMILARITY_SCORE','SUGGESTED_MAPPING']
    working_result_score_df['SIMILARITY_SCORE']:pd.Series = working_result_score_df['SIMILARITY_SCORE'].round(2)
    ...
    # single core processing
    # keyword_mapping_df['Similarity_Score1']:pd.Series = keyword_mapping_df[['TO_REPLACE','REPALCE_WITH']].apply(lambda x: self.text_scoring.get_fuzz_partial_ratio(sent_1=x[0],sent_2=x[1]),axis=1)
    # keyword_mapping_df['Similarity_Score2']:pd.Series = keyword_mapping_df[['TO_REPLACE','REPALCE_WITH']].apply(lambda x: self.text_scoring.get_jaro_winkler_similarity(sent_1=x[0],sent_2=x[1]),axis=1)

    # using pandarallel for multiprocessing
    keyword_mapping_df['Similarity_Score1']:pd.Series = keyword_mapping_df[['TO_REPLACE','REPALCE_WITH']].parallel_apply(lambda x: self.text_scoring.get_fuzz_partial_ratio(sent_1=x[0],sent_2=x[1]),axis=1)
    keyword_mapping_df['Similarity_Score2']:pd.Series = keyword_mapping_df[['TO_REPLACE','REPALCE_WITH']].parallel_apply(lambda x: self.text_scoring.get_jaro_winkler_similarity(sent_1=x[0],sent_2=x[1]),axis=1)

    keyword_mapping_df['Similarity_Score1']:pd.Series = keyword_mapping_df['Similarity_Score1'].round(2)
    keyword_mapping_df['Similarity_Score2']:pd.Series = keyword_mapping_df['Similarity_Score2'].round(2)
